...
- All content is hardcoded in `lib/content.ts` — **single source of truth**. Never scatter content into component files.
...
app/
  ...
  behavior/
    ...
    [id]/page.tsx             # Screens 3 & 4: BP detail
  decision/
    page.tsx                  # Screen 6: Decision principles list
    [id]/page.tsx             # Screens 7 & 8: DP detail
  goodbye/
    page.tsx                  # Screen 9: Goodbye
components/
...
**Dynamic routes have `dynamicParams = false`.** `app/behavior/[id]/page.tsx` and `app/decision/[id]/page.tsx` both export `dynamicParams = false` alongside `generateStaticParams`. This makes Next.js return 404 for any id outside `1` and `2` — avoids accidental hydration of invalid pages.
...
**`lib/content.ts` is the only place content lives.** If a screen's text, button label, or navigation href needs changing, it changes only in `lib/content.ts`. Never hardcode content strings in component or page files.
...
**Frontend design skill:** Use `UI UX PRO MAX` before writing any JSX/TSX for visual components (Tasks 6–14 of the plan). The UI must look like enterprise SaaS — not a template, not a slide deck.

**Implementation plan:** `/Users/moeezmujahid/.claude/plans/imperative-roaming-alpaca.md`

**Scaffold command (Task 1):** `/init`
